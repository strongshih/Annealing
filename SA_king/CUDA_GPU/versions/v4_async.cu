...
int main (int argc, char *argv[]) 
{
	...
	CUDA_CALL( cudaMemcpyAsync(couplings_buf, couplings, N*9*sizeof(int), cudaMemcpyHostToDevice) );
	...
	for (int i = 0; i < N; i++)
		initRand[i] = i;
	CUDA_CALL( cudaMemcpyAsync(randvals, initRand, N*sizeof(uint), cudaMemcpyHostToDevice) );
	...
	for (int x = 0; x < TIMES; x++) {
		...
		CUDA_CALL( cudaMemcpyAsync(spins, spins_buf, N*sizeof(int), cudaMemcpyDeviceToHost) );
