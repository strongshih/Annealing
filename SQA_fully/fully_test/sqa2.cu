							   ...
							   int randnum)
{
	...
	int idx = which_spin*M+which_layer;
	int upper = (which_layer == 0 ? M-1 : which_layer-1);
	int lower = (which_layer == M-1 ? 0 : which_layer+1);
	float delta = 2.*(float)M*(float)spins[idx]*
			((float)sigmas[idx] - (float)M*J_perp*(float)(spins[which_spin*M+upper] + spins[which_spin*M+lower]));

	if ( (-log(randnum / (float) RAND_MAX) / beta) > delta ) {
    	sigmas[i*M+which_layer] = sigmas[i*M+which_layer] - 2*spins[idx]*couplings_buf[which_spin*N+i];
		if (i == which_spin) {  
			spins[idx] = -spins[idx];
		}
	}
...
int main (int argc, char *argv[]) 
{
    ...
    for (int t = 0; t < TIMES; t++) {
        ...
        for (int p = 0; p < STEP; p++) {
            ...
            for (int m = 0; m < M; m++) {
                for (int n = 0; n < N; n++) {
                    update_sigmas<<<grid, block>>>(n, m, s_buf, sigma_buf, 
							                       couplings_buf, J_perp, beta, rand());
                }
            }
            beta += increase;
            clock_t end = clock();
