...
int main (int argc, char *argv[]) {
    ...
    status = clGetDeviceIDs(platform_id[1], CL_DEVICE_TYPE_ALL, MAXDEVICE, DEVICES, 
    ...
    for (int x = 0; x < TIMES; x++) {
        ...
        for (int t = 0; t < SWEEP; t++) {
            ...
            Jtrans[0] = -(float)M*0.5*log(tanh((Gamma/M)*beta))/beta;
			...
			for (int i = 0; i < N; i++) {
				energy += -spin_out[i] * couplings[i*N+i];
				for (int j = i+1; j < N; j++) 
					energy += -spin_out[i] * spin_out[j] * couplings[i*N+j];
			}
			assert(status == CL_SUCCESS);
			printf("Energy: %d\n", energy);
#endif
