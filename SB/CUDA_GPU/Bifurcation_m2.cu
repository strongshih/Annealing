...
__global__ void UpdateY1 (Point *p,
                         int* couplings, 
                         float amplitude)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void UpdateY2 (Point *p,
                         int* couplings, 
                         float amplitude)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (int s = 0; s < STEP; s++) {
            ...
            for (int m = 0; m < M; m++) {
                UpdateX<<<grid, block>>>(points_buf, couplings_buf, 
                                             amplitude);
                UpdateY1<<<grid, block>>>(points_buf, couplings_buf, 
                                             amplitude);
            }
            UpdateY2<<<grid, block>>>(points_buf, couplings_buf, 
                                         amplitude);
            // clock_t end = clock();
